  ...
  ws.addEventListener("message", incomingMessage => {
    try {
      ...
      if (data.type === "usernameValidation") {
        ...
        peopleList.innerHTML = "";
        const filteredActiveUsernames = activeUsernames.filter(user => user !== ws.username);

        // Включение юзеров в userListHTML
        userListHTML = filteredActiveUsernames.map(user => {
          return `
              <li class="clearfix">
                <div class="about">
        ...
        chatHistoryMessages.forEach(val => {
          ...
          chatSpace.innerHTML += `
            <li class="clearfix">
              <div class="${isCurrentUser ? "message-data text-right" : "message-data text-left"}">
                <span class="message-data-time">${val.username},${time}, ${created}</span>
              </div>
              <div class="message ${isCurrentUser ? "other-message float-right" : "other-message float-left"}">${message}</div>
            </li>`;
        });
      } else {
